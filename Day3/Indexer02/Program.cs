...
namespace Indexer02
{
    ...
    class AniTest
    {
        static void Main()
        {
            ...
            Animation ani = new Animation(6, "한국애니메이션", "10-27-2018", 35000);

            // 인스턴스화 된 ani 객체 내부의 indexer(this[index] 모양)를 사용해 title 배열의 number 번째 자리에 영화 제목을 삽입
            ani[0] = "인어공주";
            ani[1] = "신데렐라";
            ani[2] = "백설공주";
            ani[3] = "바보온달";
            ...
            {
                // 인스턴스화 된 ani 객체 내부의 indexer(this[index] 모양)를 사용해 배급번호와 영화 제목을 출력
                Console.WriteLine("배급번호 {0} : {1}", i, ani[i]);
            }
        }
    }
}
